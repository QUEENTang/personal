(function (window) {
  ...
  window.restApi = {
    get: function (path, callback, error) {
      ...
      if (path === '/todos') {
        callback.call(null, getTodos()) // eslint-disable-line 
      } else {
        if (error) {
          error.call(null, '404 Not Found') // eslint-disable-line 
        }
      }
    },
    post: function (path, body, callback, error) {
      checkCallback(callback, error)
      if (path === '/todo') {
        callback.call(null, createTodo(body)) // eslint-disable-line 
      } else {
        if (error) {
          error.call(null, '404 Not Found') // eslint-disable-line 
        }
    ...
    'delete': function (path, callback, error) {
      ...
      if (path === '/todo/completed') {
        callback.call(null, removeCompleted()) // eslint-disable-line 
      } else if (reg.test(path)) {
        var id = reg.exec(path)[1]
        var result = deleteTodo(id)
        if (result) {
          callback.call(null, result) // eslint-disable-line 
        } else {
          if (error) {
            error.call(null, '500 error') // eslint-disable-line 
          }
        }
      } else {
        if (error) {
    ...
    put: function (path, body, callback, error) {
      ...
      if (path === '/todos') {
        ...
        var id = reg.exec(path)[1]
        body.id = id
        var result = updateTodo(body) // eslint-disable-line 
        if (result) {
          callback.call(null, result) // eslint-disable-line 
        } else {
          if (error) {
